...
const OrderDetails = () => {
    const {id} = useParams();
    const {data: orderDetails, isPending, error} = useFetch('https://test-api-5zla.onrender.com/orders/' + id);
    const navigate = useNavigate();

    const handleClick = () => {
        fetch('https://test-api-5zla.onrender.com/orders/' + orderDetails.id,{
            method:'DELETE'
        }).then(() => {
            navigate('/hamro-ghar-app/');
        })
    }
                ...
                <div className="order-details-frame">
                    ...
                    <div className = "order-btn-block">
                        <div className="edit-order-btn">
                            <Link to={`/hamro-ghar-app/edit/${orderDetails.id}`}>
                                Edit
                            </Link>
                        </div>
